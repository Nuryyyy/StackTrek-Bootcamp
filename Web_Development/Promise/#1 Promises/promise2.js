const datas = [
 ...
 //from youngest tp eldest
 const sortedByAge = Promise.resolve(datas.sort((a, b) => {return (a.age - b.age)}));
 //console.log(sortedByAge)
 
 //removed the minors
 const adult = Promise.resolve(datas.filter(adult => {return adult.age >= 18
  }));
  let result = Array.isArray(adult)
 console.log(result)
 

 // renaming starts
//  const rename = adult.map(adjustName)
//  function adjustName (name) {
//      nameOnly = (name.fullName) //to access the fullName object
//      //console.log(x)
 
//      //split. remove the _ from name
//      var splits = nameOnly.split('_')
//      //console.log(splits)
 
//      for (var i = 0; i < splits.length; i++) {
//          splits[i] = splits[i].charAt(0).toUpperCase() + splits[i].slice(1);
//          } 
//      const joinAgain = splits.join(" ");
//      return joinAgain //Join the first name and last name
 
//  }
// console.log(rename)

sortedByAge.then((message) => console.log(message))
adult.then((message) => console.log(message))
//rename.then((message) => console.log(message))
